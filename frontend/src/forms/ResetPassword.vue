...
const submitForm = async () => {
  const response = await userStore.sendPasswordReset(email, token, password.value);
  if (response === true) {
...
<template>
  <BaseForm ref="baseForm" @submit="submitForm" :disabled="success">
    ...
    <input
      ...
      required
    />
    <small v-if="success" class="success">You can now log in with your new password!</small>
  </BaseForm>
</template>
@/stores/user.store

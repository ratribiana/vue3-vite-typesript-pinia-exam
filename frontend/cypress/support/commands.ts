...
declare global {
  namespace Cypress {
    interface Chainable {
      ...
      invalidFor(subject: any, options?: Partial<TypeOptions>): Chainable<Element>;
...
Cypress.Commands.add(
  ...
  (subject, options: string[]) => {
    ...
    for (const option in availableValidityOptions) {
      optionsToCheck[availableValidityOptions[option]] = options.indexOf(availableValidityOptions[option]) > -1;
    }
